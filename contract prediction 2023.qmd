...
```{r decision_tree_dep, echo=FALSE, fig.align='center',out.width="60%"}
...
anti_join(current_fa,three_year_rolling_stats_and_shares) %>% 
  select(player,type) %>% gt()
```
...
#| layout-ncol: 2
results %>% group_by(yrs_Y1S2) %>% tally() %>% gt()
results %>% group_by(yrs_S1Y2) %>% tally() %>% gt()
```
...
For the 2023-24 season, the salary cap is `r scales::dollar(current_salary_cap)`. The minimum contract from [RealGM](https://basketball.realgm.com/nba/info/salary_cap) is \$1,774,999. Therefore the minimum first year percent of cap in 2023-2024 is `r scales::percent(1774999/current_salary_cap,accuracy=0.01)` .
...
#| layout-ncol: 2
results_mins_conversion %>% group_by(yrs_Y1S2) %>% tally() %>% gt()
results_mins_conversion %>% group_by(yrs_S1Y2) %>% tally() %>% gt()
```
...
```{r gt_format_for_pdf,include=FALSE}
fa_table_format<-function(df,options=TRUE){
  formatted=df %>% select(-player_id) %>% gt() %>%
  fmt_percent(columns=c(yr1_cap_percent_Y1S2,yr1_cap_percent_S1Y2)) %>%
  fmt_currency(columns=c(total_Y1S2,total_S1Y2),suffixing=TRUE) %>%
  tab_spanner(label="Y1S2",columns=c(yr1_cap_percent_Y1S2,yrs_Y1S2,total_Y1S2)) %>%
  tab_spanner(label="S1Y2",columns=c(yr1_cap_percent_S1Y2,yrs_S1Y2,total_S1Y2)) %>%
  cols_label(yr1_cap_percent_Y1S2="Cap %",yr1_cap_percent_S1Y2="Cap %",
             yrs_Y1S2="Yrs",yrs_S1Y2="Yrs",
             total_Y1S2="Total",total_S1Y2="Total")
  if (options){
    formatted=formatted %>% fmt_currency(columns=option_amt,suffixing=TRUE)
  }
  return(formatted)
}
```
...
options_decisions=inner_join(final_results,current_fa_options) %>%
  ...
  select(player_id,player,
         yr1_cap_percent_Y1S2,yrs_Y1S2,total_Y1S2,
         yr1_cap_percent_S1Y2,yrs_S1Y2,total_S1Y2,
         option_type,option_amt)
```

```{r player_options,echo=FALSE}
fa_table_format(options_decisions %>% 
                  filter(option_type=="Player") %>% select(-option_type))
```
...
```{r team_options,echo=FALSE}
fa_table_format(options_decisions %>% 
                  filter(option_type=="Club") %>% select(-option_type))
```

Kenyon Martin Jr and Xavier Tillman Sr are in the same boat, as 2020 second round picks who signed 3-year deals (coincidentally, both were picked by the Sacramento Kings and traded to the Rockets & Grizzlies respectively). Their teams could pick up the option, but would make the players unrestricted free agents in 2024. The Rockets are intimately familiar with the process, as they declined Jae'Sean Tate's option last year to re-sign him to a [3-year, \$22.1M contract](https://rocketswire.usatoday.com/2022/06/30/rockets-re-sign-jaesean-tate-for-three-years-22-1-million/) (I had him pegged for a 2-year, \$16.7M contract). The Pelicans have a somewhat easier decision with Herb Jones, as he'd still be a restricted free agent in 2024 if the team picks up his option.

### Selected Free Agents

```{r non_option_contracts,echo=FALSE}
...
```{r rfas_top_five,echo=FALSE}
fa_table_format(non_options %>% filter(type=="RFA") %>% select(-type) %>% slice(1:5),
                options=FALSE)
```
...
```{r rfas_five_to_ten,echo=FALSE}
fa_table_format(non_options %>% filter(type=="RFA") %>% select(-type) %>% slice(6:10),
                options=FALSE)
```
...
```{r ufas_last_five,echo=FALSE}
fa_table_format(non_options %>% filter(type=="UFA") %>% select(-type) %>% slice(11:15),
                options=FALSE)
```

Strus & Gabe Vincent (average projection of 2 years and \$10.6M) were integral parts of the supporting cast flanking Jimmy Butler & Bam Adebayo during Miami's surprising run as an eight seed to the NBA Finals. They'll get more than projections after performing under the brightest of lights. While Brooks might take too many shots for someone as efficient as he is and talked his way out of Memphis (seriously, [he will not be brought back under any circumstances](https://twitter.com/ShamsCharania/status/1653430519315714049)), he is a ferocious and versatile defender. He made second-team All-Defense this year with a key part of that candidacy being that ["he has matched up against All-Stars more than any other NBA player, and he has defended them better than anybody else."](https://twitter.com/SpacesGrizzlies/status/1637440197943083010/)

```{r save_as_csvs,include=FALSE}
write_csv(options_decisions,"Output Data/Options.csv")
write_csv(non_options,"Output Data/Non-Option Contracts.csv")
```

```{r save_models,include=FALSE}
saveRDS(fit_y1_forest,"Output Data/Y1 Forest.rds")
saveRDS(fit_s2_forest,"Output Data/S2 Forest.rds")
saveRDS(fit_s2_svm,"Output Data/S2 SVM.rds")
saveRDS(fit_s1_forest,"Output Data/S1 Forest.rds")
saveRDS(fit_y2_forest,"Output Data/Y2 Forest.rds")
```
