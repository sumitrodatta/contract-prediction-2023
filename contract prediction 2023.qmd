...
final_results %>% group_by(yrs_Y1S2) %>% tally() %>% gt()
final_results %>% group_by(yrs_S1Y2) %>% tally() %>% gt()
```
...
Another task is to make the information more digestible. Contracts are usually reported in terms of total value, and players usually get five percent raises per year, so I'll add a column for each method for total contract value assuming the projected cap value of `r scales::dollar(current_salary_cap)`.

```{r total_val_add}
final_results=final_results %>% 
  mutate(total_Y1S2=
           round(yr1_cap_percent_Y1S2*current_salary_cap*((1.05)^(yrs_Y1S2)-1)/0.05,digits=-4),
         total_S1Y2=
             round(yr1_cap_percent_S1Y2*current_salary_cap*((1.05)^(yrs_S1Y2)-1)/0.05,digits=-4))
```
